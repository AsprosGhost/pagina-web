...
export function prepareRequest({product, mode, amount, institution}) {
  ...
  if (!['pension','amount','capacity','advice'].includes(mode)) throw new Error('Selecciona cómo quieres consultar.');
  if (mode !== 'advice' && (typeof amount !== 'number' || !Number.isFinite(amount) || amount <= 0)) throw new Error('Escribe una cantidad mayor que cero.');
  ...
  return {product:selected.label, mode, amount:mode==='advice'?null:amount, institution:institution.trim(), status:'selection-only', estimatedLoan:null, installment:null};
}
